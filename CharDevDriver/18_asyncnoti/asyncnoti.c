...
struct key_dev
{
    ...
    struct fasync_struct *fasunc_queue;
};
...
int key_fasync (int fd, struct file * file, int on)
{
    ...
    return fasync_helper(fd, file, on, &dev->fasunc_queue);
}
...
void timer_callbackfn(unsigned long arg)
{
    ...
    /*有效的按键过程*/
    if(atomic_read(&key.relesval))
    {
        if(dev->fasunc_queue)
        {
            kill_fasync(&dev->fasunc_queue, SIGIO, POLL_IN);
        }
    }
...
static int __init ___key_init(void)
{
    int ret = 0;
    key.major = 0;
